...
void drawPlots()
{
  ...
  drawLatex(0.2,0.77,Form("integral Lumilosity = %0.3f fb^{-1}", Lumi));
  drawLatex(0.2,0.72,Form("Counts = %0.2f M",h2D0->GetEntries()/1000000 ));
  gPad->SetLogz();
  addpdf(pdf);
  ...
  drawLatex(0.2,0.77,Form("integral Lumilosity = %0.3f fb^{-1}", Lumi));
  drawLatex(0.2,0.72,Form("Counts = %0.2f",h2D0Pi->GetEntries()/1000000 ));
  gPad->SetLogz();
  addpdf(pdf);
  ...
  drawParticle( "hLambda", "#Lambda^{0}", "#bar{#Lambda}",Lumi, pdf, f);
